...
import { getWikipediaPage, getImageFromFilename, WIKIPEDIA_USER_AGENT, getEntity } from "../_shared/extract/constants.ts";
...
export default {
  fetch: withSupabase<Database>({ auth: "user" }, async (req, ctx) => {
    try {
      ...
      if (!wikipediaUrl) {
        return Response.json({ ok: false, error: "wikipediaUrl is required" }, { status: 400 });
      }
      ...
      try {
        ...
        return Response.json({ ok: false, error: "Invalid Wikipedia URL" }, { status: 400 });
      }
      ...
      if (!data?.query?.pages) {
        return Response.json({ ok: false, error: "Wikipedia page not found" }, { status: 404 });
      }
      ...
      if (wikidataId) {
        ...
        const imageUrlRes = await fetch(getImageFromFilename(pageprops.page_image_free), {
          headers: { "User-Agent": WIKIPEDIA_USER_AGENT }
        });
        const imageUrlData = await imageUrlRes.json();
        const imagePages = imageUrlData?.query?.pages;
