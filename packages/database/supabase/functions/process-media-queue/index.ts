...
export default {
  fetch: withSupabase<Database>({ auth: "secret:*" }, async (req, ctx) => {
    try {
      ...
      // Process up to 5 items to avoid function timeout
      for (let i = 0; i < 5; i++) {
        // Pop a message from the queue using public.pop_media_queue_message RPC
        ...
        try {
          // Invoke prepare_movie for this queue item
          const response = await fetch(
            `${SUPABASE_URL}/functions/v1/prepare_movie`,
            {
              method: "POST",
              headers: {
