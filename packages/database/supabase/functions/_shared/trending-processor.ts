...
export function validateEnvironment(): {
  ...
  const requiredVars = ["TMDB_API_KEY", "SUPABASE_URL"];
...
export async function fetchTrendingMedia(
  ...
  if (mediaType === "both") {
    ...
    if (!moviesResponse.ok) {
      throw new Error(
        `Movies API request failed: ${moviesResponse.status} ${moviesResponse.statusText}`,
      );
    }
    if (!showsResponse.ok) {
      throw new Error(
        `TV Shows API request failed: ${showsResponse.status} ${showsResponse.statusText}`,
      );
    }
...
export async function processMediaItems(
  ...
  console.log(
    `Starting sequential processing of ${itemsToProcess.length} ${config.mediaType} items.`,
  );
  ...
  for (const media of itemsToProcess) {
    ...
    const result = await (async (): Promise<ProcessingResult> => {
      ...
      const functionUrl =
        typeof config.prepareFunctionUrl === "string"
          ? config.prepareFunctionUrl
          : config.prepareFunctionUrl[media.type];

      console.log(
        `Invoking ${functionUrl} for ${media.type} ${tmdbId} (${title})`,
      );
      ...
      console.log(
        `DEBUG: Sending parameters to ${
          isShowFunction ? "show" : "prepare_movie"
        } function:`,
        requestBody,
      );
      ...
      try {
        ...
        if (result.ok === false) {
          throw new Error(
            result.error || `${config.prepareFunctionUrl} returned an error`,
          );
        }
        ...
        console.error(
          `Failed to process ${config.mediaType} ${tmdbId} (${title}):`,
          errorMessage,
        );
...
export function generateSummary(results: ProcessingResult[]): {
  ...
  const successfulMovies = results.filter(
    (r) => r.status === "fulfilled" && r.value?.type === "movie",
  );
  const successfulShows = results.filter(
    (r) => r.status === "fulfilled" && r.value?.type === "tv",
  );
...
export async function processTrendingMedia(
  ...
  try {
    ...
    if (!envValidation.isValid) {
      throw new Error(
        `Missing environment variables: ${envValidation.missingVars.join(
          ", ",
        )}`,
      );
    }
