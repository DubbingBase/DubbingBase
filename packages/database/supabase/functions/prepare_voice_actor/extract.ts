...
const extractMatchingSection = async (
  ...
  const responseParseSection = (await wikipediaCache.getPageContentAsWikitext(
    element.pageid,
    sectionIndex,
  )) as ResponseParseText;
...
export const exploreDubbingSectionChilds = async (
  ...
  if (sectionLength === 0) {
    ...
    const result = await extractMatchingSection(
      dubbingSectionIndex,
      toc,
      element,
    );
    ...
    for (const section of sectionToExplore.sections) {
      ...
      if (isAcceptedDubbingSubSection) {
        ...
        const internalResults = await exploreDubbingSectionChilds(
          section.index,
          toc,
          element,
        );
