...
export const isMultiLineLi1 = /\[\[(?<actor_name>.*)\]\]  ?dans(?:.*)[:;] ?$/i;
export const isMultiLineLi1_1 =
  /\[\[(?<actor_name>.*)\]\] \(.*\) dans(?:.*)[:;] ?$/i;
export const isMultiLineLi1_2 = /\[\[(?<actor_name>.*)\]\] dans \((\d+.*)\)$/i;
export const isMultiLineLi1_3 =
  /\[\[(?<actor_name>.*)\]\] dans [:;] \((\d+.*)\)$/i;
export const isMultiLineLi2 = /(?<actor_name>.*) dans(?:.*): ?$/i;
export const isMultiLineLi2_1 = /dans ''\[\[(?<media_title>.*)\]\]'' [:;] ?$/i;
export const isMultiLineLi3 =
  /Dans la série ''\[\[(?<media_title>.*)\]\]'' ?$/i;
export const isMultiLineLi4 = /\[\[(?<actor_name>.*)\]\] :$/i;
export const isMultiLineLi5 = /''\[\[(?<media_title>.*)\]\]'' : ?$/i;
export const isMultiLineLi6 = /\[\[(?<year>[\d\-\[\]]+)\]\] : ?$/i;
...
export const singlelineMatcherVideoGame = [singlelineMatcherVideoGame1];

export const matchersSingleLineByType: Record<
  ElementType,
  (string | RegExp)[][]
> = {
...
export const preProcess: Array<[string, RegExp, string]> = [
  ...
  [
    "indicateur-langue",
    /<span(.*?)class="indicateur-langue"(.*)<\/span>/gi,
    "",
  ],
  ...
  [
    "remove",
    /Les dates en italique correspondent aux sorties initiales des films dont Jean-Luc Kayser a assuré le redoublage/gi,
    "",
  ],
  ...
  [
    "skip",
    /\[\[2013]] : ''\[\[Arrested Development]]'' : l'acteur gay.*/gi,
    "",
  ],
