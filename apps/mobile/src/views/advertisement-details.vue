...
import { Megaphone, Calendar, Building2 } from "lucide-vue-next";
...
async function fetchData() {
  ...
  try {
    ...
    const data = await fetchAdvertisementData(adId, locale.value);
  ...
  <AppPage>
    <AppHeader>
      <AppToolbar>
        ...
        <AppTitle>{{ advertisement?.title || $t("search.advertisement", "Publicité") }}</AppTitle>
      </AppToolbar>
    </AppHeader>
    ...
    <AppContent>
      ...
      <!-- Content -->
      <div v-else-if="advertisement" class="pb-8">
        <!-- Hero Header -->
        <div class="relative bg-gray-900 px-4 pt-4 pb-6">
          <div class="flex gap-4 items-start">
            <!-- Cover / Poster -->
            <div class="w-28 shrink-0 rounded-xl overflow-hidden shadow-lg aspect-video bg-gray-800 flex items-center justify-center">
              <img
                v-if="advertisement.poster_url"
                :src="advertisement.poster_url"
                :alt="advertisement.title"
                class="w-full h-full object-cover"
              />
              <Megaphone v-else class="w-10 h-10 text-gray-500" />
            </div>

            <!-- Meta info -->
            <div class="flex-1 min-w-0 space-y-2">
              <div class="flex items-center gap-1.5 text-xs text-emerald-400 font-semibold uppercase tracking-wider">
                <Megaphone class="w-3.5 h-3.5" />
                <span>{{ $t("search.advertisement", "Publicité") }}</span>
              </div>
              ...
              <div v-if="advertisement.brand" class="flex items-center gap-1.5 text-xs text-gray-300">
                <Building2 class="w-3.5 h-3.5 text-gray-400 shrink-0" />
                <span class="truncate">{{ advertisement.brand }}</span>
              </div>

              <div v-if="advertisement.year" class="flex items-center gap-1.5 text-xs text-gray-400">
                <Calendar class="w-3.5 h-3.5 shrink-0" />
                <span>{{ advertisement.year }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="advertisement.description" class="px-4 mt-6 mb-6">
