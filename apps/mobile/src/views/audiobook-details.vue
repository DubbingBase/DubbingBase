...
import type { Audiobook } from "@app/shared-logic";
...
async function fetchData() {
  ...
  try {
    ...
    const data = await fetchAudiobookData(bookId, locale.value);
    if (data && data.audiobook) {
      audiobook.value = data.audiobook;
  ...
  <AppPage>
    <AppHeader>
      <AppToolbar>
        ...
        <AppTitle>{{ audiobook?.title || $t("search.audiobook", "Livre audio") }}</AppTitle>
      </AppToolbar>
    </AppHeader>
    ...
    <AppContent>
      ...
      <!-- Content -->
      <div v-else-if="audiobook" class="pb-8">
        <!-- Hero Header -->
        <div class="relative bg-gray-900 px-4 pt-4 pb-6">
          <div class="flex gap-4 items-start">
            <!-- Cover Poster -->
            <div class="w-28 shrink-0 rounded-xl overflow-hidden shadow-lg aspect-[2/3] bg-gray-800 flex items-center justify-center">
              <img
              ...
              />
              <BookOpen v-else class="w-10 h-10 text-gray-500" />
            </div>
            ...
            <div class="flex-1 min-w-0 space-y-2">
              <div class="flex items-center gap-1.5 text-xs text-amber-400 font-semibold uppercase tracking-wider">
                ...
                <span>{{ $t("search.audiobook", "Livre audio") }}</span>
              </div>
              ...
              <div v-if="authorsText" class="flex items-center gap-1.5 text-xs text-gray-300">
                <User class="w-3.5 h-3.5 text-gray-400 shrink-0" />
                <span class="truncate">{{ authorsText }}</span>
              </div>

              <div v-if="audiobook.first_publish_date" class="flex items-center gap-1.5 text-xs text-gray-400">
                <Calendar class="w-3.5 h-3.5 shrink-0" />
                <span>{{ audiobook.first_publish_date }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="audiobook.description" class="px-4 mt-6 mb-6">
