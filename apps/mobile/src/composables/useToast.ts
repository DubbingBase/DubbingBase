    ...
    return {
      present: async () => {
        ...
        if (options.color === "danger")
          toast.error(options.message, { duration: durationMs });
        else if (options.color === "success")
          toast.success(options.message, { duration: durationMs });
        else if (options.color === "warning")
          toast.warning(options.message, { duration: durationMs });
...
export function useToast() {
  const showToast = async (
    ...
    const toastDuration =
      duration === "short" ? 2000 : duration === "long" ? 3500 : duration;
