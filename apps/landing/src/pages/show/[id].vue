<template>
  <div v-if="serie" class="bg-[#1b1b1b] min-h-screen text-white">
    <!-- Hero Section -->
      ...
      <div class="absolute bottom-0 left-0 w-full p-8 flex flex-col md:flex-row gap-6 items-end">
        ...
        <div class="pb-4">
          ...
          <p class="text-gray-300 mt-2 text-lg">
            ...
            <span v-if="serie.number_of_seasons"> &bull; {{ serie.number_of_seasons }} Saisons</span>
          </p>
        </div>
      </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div class="flex flex-col items-center border-b border-[#2a2a2a] pb-3 mb-2">
              ...
              <div class="text-center text-sm font-semibold text-gray-400 truncate w-full" :title="actor.roles?.map((r: any) => r.character).join(', ') || ''">
                {{ actor.roles?.map((r: any) => r.character).join(', ') || 'Personnage inconnu' }}
              </div>
            </div>
...
const formattedCast = computed(() => {
  ...
  return aggregateCredits.value.cast.map((actor: any) => {
    ...
    const characterName = actor.roles?.map((r: any) => r.character).join(', ');
    if (characterName) {
      // Try to match any of the roles
      for (const role of actor.roles || []) {
        if (!role.character) continue;
        const match = characterProfilePictures.value.find((c: any) =>
          c.name && c.name.toLowerCase() === role.character.toLowerCase()
        );
...
useHead({
  title: computed(() => {
    let base = serie.value ? `${serie.value.name}` : 'Série';
