<template>
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <form @submit.prevent="saveMovieProject" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5 h-fit shadow-xl">
          ...
          <!-- Content ID / TMDB ID -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">TMDB / Content ID *</label>
            <div class="flex space-x-2">
              <input
                ...
                required
                placeholder="e.g. 550"
                class="w-full px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
              />
              <button
                type="button"
          ...
          <!-- Content Type -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Content Type</label>
            <select
              v-model="contentType"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="space-y-1">
              <div class="flex justify-between items-center">
                ...
                <router-link to="/studios/new" target="_blank" class="text-[10px] text-blue-400 hover:underline">+ New Studio</router-link>
              </div>
              <div class="flex space-x-2">
                <select
            ...
            <!-- Mixing -->
            <div class="space-y-1">
              <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Sound Mixing (Mixage)</label>
              <input
                v-model="mixing"
                type="text"
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl">
        <div class="flex justify-between items-center border-b border-slate-800 pb-3">
          <div>
            ...
            <p class="text-xs text-slate-400">Map original actors to french voice actors and character names.</p>
          </div>
          <div class="flex items-center space-x-2">
            <button
              type="button"
              @click="openCreatePersonModal"
              class="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-blue-400 hover:text-blue-300 font-semibold rounded-xl text-xs border border-slate-700 transition-all flex items-center space-x-1"
            >
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-sm text-slate-300">
            ...
            <tbody class="divide-y divide-slate-800/60">
              <tr v-for="(row, index) in castRows" :key="index" class="hover:bg-slate-950/50 transition-colors">
                <!-- Actor ID -->
                <td class="px-4 py-3 w-28">
                  <input
                    ...
                    class="w-full px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-lg text-white text-xs focus:ring-2 focus:ring-blue-500"
                  />
                </td>
                ...
                <td class="px-4 py-3 text-center w-20">
                  <input
                    ...
                    type="checkbox"
                    class="h-4 w-4 rounded border-slate-800 bg-slate-950 text-blue-600 focus:ring-blue-500"
                  />
                </td>
    ...
    <div v-if="showCreatePersonModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full space-y-4 shadow-2xl">
        <div class="flex justify-between items-center border-b border-slate-800 pb-3">
          ...
          <button @click="showCreatePersonModal = false" class="text-slate-400 hover:text-white text-lg">✕</button>
        </div>

        <div class="space-y-3">
          <div>
            <label class="text-xs font-semibold text-slate-400 uppercase">First Name *</label>
            <input
              v-model="newPersonFirstname"
              type="text"
              required
              placeholder="e.g. Richard"
              class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-white text-sm focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div>
            <label class="text-xs font-semibold text-slate-400 uppercase">Last Name *</label>
            <input
          ...
          <div>
            <label class="text-xs font-semibold text-slate-400 uppercase">Nationality</label>
            <input
