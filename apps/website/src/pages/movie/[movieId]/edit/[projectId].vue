<template>
  <div class="max-w-6xl mx-auto space-y-6">
    ...
    <!-- Navigation Tabs -->
    <div v-if="tmdbMovieId" class="flex flex-wrap gap-2 pb-2 border-b border-slate-800">
      <NuxtLink
    ...
    <form v-else @submit.prevent="saveMovieProject" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5 h-fit shadow-xl">
          ...
          <!-- Content ID / TMDB ID -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">TMDB Movie ID *</label>
          ...
          <div v-for="(row, index) in castRows" :key="index" class="p-4 bg-slate-950 border border-slate-800 rounded-xl relative group">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              <div class="md:col-span-12 space-y-1">
                <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Original Actor / Character *</label>
                <AsyncAutocomplete
...
const fetchTmdbMetadata = async () => {
  ...
  if (movieIdParam === 'new') {
    router.push(localePath(`/movie/${contentId.value}/edit/new`));
    return;
  }
  
  isFetchingTmdb.value = true;
  try {
    const { data, error } = await supabase.functions.invoke("movie", { body: { id: contentId.value } });
    if (error) throw error;
    if (data?.movie) {
      mediaTitle.value = data.movie.title;
      // Handle relative paths for posters
      let poster = data.movie.poster_path;
      ...
      if (data.movie.credits?.cast) {
        tmdbCast.value = data.movie.credits.cast;
        filteredTmdbCast.value = tmdbCast.value.slice(0, 50);
      }
    }
  } catch (e: any) {
...
const loadData = async () => {
  if (tmdbMovieId.value) {
    ...
    if (projects) {
      movieDubbingProjects.value = projects;
    }
  }
