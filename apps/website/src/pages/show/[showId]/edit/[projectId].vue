    ...
    <div class="bg-gray-900 p-6 rounded-2xl border border-gray-800 flex justify-between items-center shadow-xl">
      <div>
        <h3 class="text-xl font-bold text-white flex items-center gap-2">
          ...
          {{ isEditMode ? $t('projects.editShowProject') : $t('projects.createShowProject') }}
        </h3>
      ...
      >
        <span>{{ tmdbShowId ? $t('projects.backToShow') : $t('common.backHome') }}</span>
      </NuxtLink>
    </div>
    ...
    <form v-else @submit.prevent="saveShowProject" class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-5 h-fit shadow-xl">
          ...
          <!-- Content ID / TMDB ID -->
          <div class="space-y-1">
            <label class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ $t('projects.tmdbShowId') }}</label>
      ...
      <div class="flex justify-end pt-4">
        ...
        >
          ...
          <span v-else>{{ $t('projects.saveShow') }}</span>
        </button>
      </div>
    </form>
