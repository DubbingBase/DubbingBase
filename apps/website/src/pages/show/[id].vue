<template>
  <div v-if="serie" class="bg-gray-50 dark:bg-[#1b1b1b] min-h-screen text-gray-900 dark:text-white">
    <!-- Hero Section -->
...
const filteredCast = computed(() => {
  ...
  return formattedCast.value.filter((actor: any) => {
    ...
    const characterName = (actor.roles?.map((r: any) => r.character).join(', ') || '').toLowerCase();
