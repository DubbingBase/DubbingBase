    ...
    >
      <div>
        <h3 class="text-xl font-bold theme-text flex items-center gap-2">
          ...
          {{
            isEditMode
              ? $t("admin.studios.editStudioProfile")
              : $t("admin.studios.createDubbingStudio")
          }}
        </h3>
        <p class="text-sm theme-text-muted mt-1">
          {{
            isEditMode
              ? $t("admin.studios.updatingStudioRecord", { id })
              : $t("admin.studios.fillStudioInfo")
          }}
        </p>
      </div>
      <NuxtLink
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="space-y-1 md:col-span-2">
          <label
            ...
            >{{ $t("admin.studios.studioName") }} *</label
          >
          <input
            v-model="name"
            type="text"
        ...
        <div class="space-y-1">
          <label
            class="text-xs font-semibold theme-text-muted uppercase tracking-wider"
            >{{ $t("admin.studios.websiteUrl") }}</label
          >
          <input
            v-model="websiteUrl"
            type="url"
        ...
        <div class="space-y-2">
          <label
            class="text-xs font-semibold theme-text-muted uppercase tracking-wider block"
            >{{ $t("admin.studios.studioLogo") }}</label
          >
          <div class="flex items-center gap-4">
            <div
              class="relative h-16 w-16 rounded-xl overflow-hidden border theme-border theme-input flex shrink-0 items-center justify-center theme-text-muted shadow-inner group"
            >
              <NuxtImg
                format="webp"
              ...
              <svg
                v-else
                class="h-8 w-8 theme-text-secondary"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
            ...
            <div class="flex-1 space-y-2">
              ...
              <div class="flex items-center gap-2">
                <button
                  ...
                  class="py-2 px-3 theme-surface-muted theme-hover-surface-muted theme-text text-xs font-semibold rounded-lg transition-colors border theme-border"
                >
                  {{
                    logoPreview
                      ? $t("admin.studios.changeLogo")
                      : $t("admin.studios.uploadLogo")
                  }}
                </button>
                <button
                  v-if="logoPreview || logoUrl"
                  type="button"
                  @click="clearLogo"
                  class="py-2 px-3 theme-status-danger hover:bg-[var(--app-color-danger-bg)] theme-status-danger-text text-xs font-semibold rounded-lg transition-colors border border-[var(--app-color-danger-border)]"
                >
                  {{ $t("common.remove") }}
                </button>
              </div>
      ...
      <div class="space-y-1">
        ...
        <textarea
          ...
          class="w-full px-4 py-3 theme-input border theme-border rounded-xl theme-text theme-placeholder focus:outline-none focus:ring-2 focus:ring-[var(--app-color-focus)] text-sm resize-y"
        ></textarea>
      </div>
