<template>
  <div class="space-y-6">
    ...
    >
      ...
      <div class="w-full sm:w-80 relative">
        <input
          ...
          class="w-full pl-10 pr-4 py-2.5 theme-input border theme-border rounded-xl theme-text theme-placeholder focus:outline-none focus:ring-2 focus:ring-[var(--app-color-focus)] transition-all text-sm"
        />
        <span class="absolute left-3.5 top-3 theme-text-muted">
          <svg
            class="h-4.5 w-4.5"
            fill="none"
            viewBox="0 0 24 24"
            ...
            >
              <td class="py-4 px-6">
                <div class="font-semibold theme-text">
                  {{ user.email || $t("admin.users.anonymousUser") }}
                </div>
              ...
              <td class="py-4 px-6 font-mono text-xs theme-text-muted">
                {{ user.id }}
              </td>
              <td class="py-4 px-6">
                <select
                  ...
                  class="theme-input border theme-border rounded-xl px-3 py-1.5 text-xs font-semibold theme-text focus:outline-none focus:ring-2 focus:ring-[var(--app-color-focus)] disabled:opacity-50"
                >
              ...
              <td class="py-4 px-6 text-right">
                <button
                  ...
                  class="px-3 py-1.5 theme-status-danger hover:bg-[var(--app-color-danger-bg)] theme-status-danger-text hover:text-[var(--app-color-danger-text)] border border-[var(--app-color-danger-border)] hover:border-red-900/50 rounded-xl text-xs font-semibold transition-all"
                >
                  {{ $t("common.delete") }}
                </button>
              </td>
    ...
    <div
      v-if="showConfirm"
      class="fixed inset-0 z-50 flex items-center justify-center p-4 theme-input backdrop-blur-md"
    >
      <div
        class="theme-surface-overlay border theme-border rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4"
      >
        <div class="flex items-center space-x-3 theme-status-danger-text">
          <svg
            class="h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
        ...
        <p class="text-sm theme-text-secondary">
          ...
          }}<strong class="theme-text">{{ userToDelete?.email }}</strong
          >{{ $t("admin.users.irreversibleAction") }}
        </p>
        <div class="flex justify-end space-x-3 pt-2">
          <button
            @click="showConfirm = false"
            class="px-4 py-2 theme-surface-muted theme-hover-surface-muted theme-text-secondary rounded-xl text-sm font-semibold border theme-border transition-colors"
          >
...
const deleteUser = async () => {
  ...
  try {
    ...
    showToast(
      `Utilisateur ${userToDelete.value.email} supprimé avec succès`,
      "success",
    );
