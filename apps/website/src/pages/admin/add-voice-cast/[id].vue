<template>
  <div class="space-y-6">
    <!-- Top back banner -->
    <div
      class="flex items-center justify-between theme-surface-overlay p-4 rounded-2xl border theme-border"
    >
      <h3 class="text-sm font-bold theme-text">
        {{ $t("admin.addVoiceCast.title") }}
      </h3>
      <NuxtLink
        :to="localePath('/admin')"
        class="text-xs font-semibold px-4 py-2 theme-surface-muted theme-hover-surface-muted theme-text-secondary theme-hover-text rounded-xl border theme-border transition-colors"
    ...
    <div
      v-else-if="mediaSummary"
      class="theme-surface-overlay border theme-border rounded-2xl p-5 space-y-4 shadow-xl"
    >
      <div class="flex gap-4">
        <div
          class="h-32 w-24 rounded-lg overflow-hidden border theme-border theme-input shrink-0 flex items-center justify-center theme-text-secondary"
        >
          <img
            v-if="mediaSummary.poster"
            :src="mediaSummary.poster"
      ...
      <div class="border-t theme-border pt-4 space-y-3">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <input
            ...
            class="flex-1 px-3 py-2 theme-input border theme-border rounded-xl theme-text text-sm focus:ring-2 focus:ring-[var(--app-color-focus)]"
          />
          <select
            v-model="manualPerformance"
            class="px-3 py-2 theme-input border theme-border rounded-xl theme-text text-sm"
          >
        ...
        <div
          v-if="manualVAOptions.length > 0"
          class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto"
        >
          <button
            ...
            :class="
              selectedManualVA?.id === va.id
                ? 'bg-indigo-950/40 border-indigo-700'
                : 'theme-input theme-border theme-hover-surface-muted'
            "
          >
            <div
              class="h-7 w-7 rounded-full overflow-hidden border theme-border theme-input shrink-0 flex items-center justify-center theme-text-muted"
            >
              <img
        ...
        <div class="flex flex-col sm:flex-row gap-2 pt-2 border-t theme-border">
          <input
            v-model="manualNewVAFirstname"
            type="text"
            :placeholder="$t('admin.movieEditor.firstName')"
            class="flex-1 px-3 py-2 theme-input border theme-border rounded-xl theme-text text-xs"
          />
          <input
    ...
    <div v-else-if="media" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <div class="space-y-2">
            ...
            <p
              ...
              class="text-xs theme-text-muted leading-relaxed line-clamp-4 italic"
            >
              "{{ media.description }}"
            </p>
          </div>
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div
          ...
          class="p-8 text-center theme-surface-overlay border theme-border rounded-2xl theme-text-muted text-sm"
        >
          {{ $t("admin.addVoiceCast.noCastReturned") }}
        </div>
        ...
        <div v-else class="space-y-3">
          <div
            ...
            :key="actor.id"
            class="theme-surface-overlay border theme-border rounded-xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-sm"
          >
            <div class="flex items-center space-x-3.5 min-w-0">
              <div
                class="h-12 w-12 rounded-full overflow-hidden border theme-border theme-input shrink-0 flex items-center justify-center theme-text-muted"
              >
                <img
              ...
              <div class="min-w-0">
                <h5 class="font-bold theme-text text-sm truncate leading-snug">
                  {{ actor.name }}
                </h5>
            ...
            <div class="relative w-full sm:w-72 shrink-0">
              ...
              >
                <div class="min-w-0 flex items-center space-x-2">
                  <div
                    class="h-5 w-5 rounded-full overflow-hidden border border-indigo-800 theme-input shrink-0 flex items-center justify-center theme-status-info-text"
                  >
                    <img
                      v-if="getAssignedVA(actor.id)?.profile_picture"
                      :src="getAssignedVA(actor.id)!.profile_picture!"
                      class="h-full w-full object-cover"
                    />
              ...
              <div v-else class="relative">
                ...
                >
                  <div
                    ...
                    class="px-3 py-2 theme-hover-surface-muted cursor-pointer flex items-center space-x-2.5 transition-colors"
                  >
                    <div
                      class="h-6 w-6 rounded-full overflow-hidden border theme-border theme-input shrink-0 flex items-center justify-center theme-text-muted"
                    >
                      <img
                        v-if="va.profile_picture"
                        :src="va.profile_picture"
                ...
                <div
                  v-else-if="
                    activeSearchDropdown === actor.id &&
                    actorSearchQueries[actor.id]?.trim() &&
                    searchLoading[actor.id]
                  "
                  class="absolute z-40 left-0 right-0 mt-1.5 p-3 theme-surface-overlay border theme-border rounded-xl text-center theme-text-muted text-[10px]"
                >
                  {{ $t("admin.addVoiceCast.searchingDatabase") }}
                </div>
              </div>
            </div>
          </div>
...
const supportsPublicCast = computed(() =>
  ["movie", "tv", "video_game"].includes(mediaTypeParam.value),
);
...
const quickCreateVoiceActorForManual = async () => {
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert([
        {
          firstname: manualNewVAFirstname.value.trim(),
          lastname: manualNewVALastname.value.trim(),
        },
      ])
      .select()
      .single();
