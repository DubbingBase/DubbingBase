  ...
  <div class="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 min-h-screen">
    ...
    >
      <div>
        ...
        <!-- Search bar -->
        <div class="mt-4 max-w-md" v-if="totalStudios > 10">
          <div class="relative">
            <input
              v-model="searchInput"
              type="text"
    ...
    >
      <div
        v-for="i in 10"
        :key="i"
        class="w-full h-48 md:h-64 bg-gray-200 dark:bg-gray-800 animate-pulse rounded-xl"
      ></div>
    </div>
    ...
    <div v-else>
      ...
      >
        <template #default="{ item: studio }">
          ...
          >
            <div
              class="relative w-full aspect-square rounded-xl overflow-hidden mb-3 bg-gray-200 dark:bg-gray-800 flex items-center justify-center shadow-md transition-transform duration-300 group-hover:-translate-y-1 group-hover:shadow-xl"
            >
              <img
            ...
            <h3
              class="font-semibold text-sm md:text-base text-gray-800 dark:text-gray-200 line-clamp-2 group-hover:text-cyan-500 transition-colors"
            >
              {{ studio.name }}
            </h3>
            <p
