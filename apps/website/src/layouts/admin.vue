<template>
  <div
    class="flex flex-col lg:flex-row w-full min-h-screen theme-bg theme-text"
  >
    <!-- Mobile Admin Nav Bar -->
    <div
      class="lg:hidden sticky top-[72px] md:top-[80px] z-40 theme-surface-raised theme-surface border-b theme-border-subtle theme-border px-4 py-3 flex items-center justify-between shadow-sm"
    >
      <h2 class="text-sm font-semibold theme-text">{{ currentSectionName }}</h2>
      <button
        @click="isSidebarOpen = !isSidebarOpen"
        class="theme-text-muted theme-hover-text theme-text-muted theme-hover-text"
      >
        <svg
          class="h-6 w-6"
          fill="none"
    ...
    <aside
      :class="[
        'fixed inset-y-0 left-0 w-72 theme-bg lg:bg-transparent border-r lg:border-r-0 theme-border-subtle theme-border z-50 transform transition-transform duration-300 ease-in-out flex flex-col',
        isSidebarOpen ? 'translate-x-0' : '-translate-x-full',
        'lg:sticky lg:top-[80px] lg:h-[calc(100vh-80px)] lg:shrink-0 lg:translate-x-0 lg:z-30',
      ]"
    >
      <div class="h-4 hidden lg:block shrink-0"></div>
      <!-- Small top padding for desktop -->
      ...
      <nav class="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto">
        <NuxtLink
          ...
          class="flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-150 group font-medium text-sm theme-text-secondary theme-text-muted theme-hover-text theme-hover-surface-muted"
          active-class="theme-selected border theme-primary-border"
          @click="isSidebarOpen = false"
        >
